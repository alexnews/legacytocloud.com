...
## Part 5: Critical Bug Fixes (Late Session)

### Python 3.10 f-string Syntax Error
**Problem:** Backend crashed with `SyntaxError: f-string expression part cannot include a backslash`
**File:** `backend/app/services/ddl_generator.py` line 187
**Cause:** Python 3.10 doesn't allow backslashes inside f-string expressions
**Bad code:**
```python
...
### Deploy Script .htaccess Fix
**Problem:** `.htaccess` file deleted on every deploy (overwrites www folder)
**Fix:** Added to `deploy.sh` to recreate .htaccess after copying files:
```bash
